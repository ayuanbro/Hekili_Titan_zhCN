...
一、性能优化：
1.全面优化Targets.lua目标检测系统，提升姓名板和宠物范围识别性能
2.实现智能缓存机制：根据目标移动状态动态调整缓存时间，静止目标缓存更久
3.添加分帧处理：限制每帧最大处理目标数(12个)，避免单帧处理过多目标造成卡顿
4.优化扫描频率：姓名板扫描间隔调整为150ms，减少不必要的API调用
5.实现批量范围检测：异步批量处理宠物范围检测，提升检测效率

二、目标检测改进：
1.新增智能目标过滤系统：距离预过滤快速排除过远目标，减少详细检查开销
2.优化训练假人检测：缓存已知训练假人NPC ID，智能名称匹配，避免重复检测
3.职业特定优化：为猎人、术士、法师等职业提供专门的检测策略
4.增强调试功能：提供详细的缓存统计和性能监控数据

三、技术改进：
1.优化内存管理：定期清理过期缓存，防止内存泄漏
2.新增性能调优接口：提供performance/balanced/quality三种优化级别
3.完善缓存清理机制：智能清理无效GUID和过期数据
4.增强错误处理：改进目标有效性检查和异常情况处理
