...
spec:RegisterGlyphs( {
    ...
    [63326] = "vigilance",
    [58370] = "whirlwind",
    [414812] = "shattering_throw", --新增by风雪 碎裂投掷雕文
} )


-- Gear Sets
spec:RegisterGear( "tier7", 40525, 40528, 40529, 40527, 40530, 43739, 43744, 43746, 43741, 43748, 39606, 39605, 39607, 39609, 39608 )
spec:RegisterGear( "tier10berserk", 50078, 50079, 50080, 50081, 50082, 51210, 51211, 51212, 51213, 51214, 51225, 51226, 51227, 51228, 51229 ) --新增 by风雪 20250706
-- Prot
...
spec:RegisterAbilities( {
    ...
    -- 拳击 Pummel the target, interrupting spellcasting and preventing any spell in that school from being cast for 4 sec.
    pummel = {
        id = 6552,
        cast = 0,
        cooldown = 10,
        ...
        toggle = "interrupts", --加入打断组，修改by风雪 20250812
...
-- 是否应该使用破甲 by 哑吡 20251226
spec:RegisterStateExpr("should_sunder", function()
    -- 检查是否启用破甲维护
    if settings.debuff_sunder_enabled == false then return false end
    -- 检查破甲层数是否已满
    if debuff.sunder_armor.stack >= 5 then return false end
    return true
end)
...
spec:RegisterSetting("optimize_overpower", false, {
    type = "toggle",
    name = "优化压制",
    desc = "启用后, 在随后的嗜血成性前，压制将被取消优先级，直到主要爆发结束。\n\n仅适用于武器专精。",
    width = "full",
    set = function( _, val )
...
-- 新增猛击优先选项 修改by 风雪20250706
spec:RegisterSetting("slam_priority", false, {
    type = "toggle",
    name = "猛击优先(4T10)",
    desc = "使猛击(4T10)优先级提高，但可能存在增加公共冷却时间的bug 或 导致总伤害更低的问题，供玩家自行决定", 
    width = "single",
} )
-- 新增结束
