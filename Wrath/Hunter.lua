...
spec:RegisterAbilities( {
    ...
    explosive_trap = {
        id = 425777,     --49067改为425777爆炸陷阱发射器 修改 by 风雪20250705
        cast = 0,
        cooldown = function() return mod_resourcefulness_cd( 30 ) end,
        gcd = "spell",
    ...
    -- 稳固射击 A steady shot that causes unmodified weapon damage, plus ammo, plus 78.  Causes an additional 175 against Dazed targets.
    steady_shot = {
        id = 56641,
        cast = function()
            local base = 2 * state.haste / 1.15  -- 基础值考虑急速、箭袋15%加速 新增by风雪 20250901
            if buff.rapid_fire.up then base = base / 1.4 end  -- 急速射击40%加速 新增by风雪 20250901
            if buff.quick_shots.up then base = base / 1.15 end -- 快速射击15%加速 新增by风雪 20250901
            return base
        end,
    ...
    call_of_the_wild = {
        ...
        toggle = "cooldowns", -- 修改 by 风雪20250418.野性呼唤加入爆发组。 
...
spec:RegisterPackSelector( "survival", "生存(黑科研)", "|T132215:0|t 生存",
...
spec:RegisterSetting( "kill_shot_advance", 0.25, {
    ...
    desc = "|T236174:0|t杀戮射击会提前设定值进行提示(单位：秒),默认0.25。",
    width = "full",
    min = 0,
    softMax = 1.5,
    step = 0.05,
...
spec:RegisterSetting( "explosive_shot_advance", 0.7, {
    type = "range",
...
spec:RegisterSetting( "stings_advance", 0, {
    type = "range",
    name = "|T132204:0|t毒蛇钉刺提前量",
    desc = "|T132204:0|t毒蛇钉刺会提前设定值进行提示(单位：秒),默认0。",
    width = "full",
    min = 0,
    softMax = 1.5,
    step = 0.01,
...
spec:RegisterSetting( "aimed_shot_advance", 0.25, {
    type = "range",
    name = "|T135130:0|t瞄准射击提前量",
    desc = "|T135130:0|t瞄准射击会提前设定值进行提示(单位：秒),默认0.25。",
    width = "full",
    min = 0,
    softMax = 1.5,
    step = 0.05,
...
spec:RegisterSetting( "multishot_advance", 0.2, {
    type = "range",
    name = "|T132330:0|t多重射击提前量",
    desc = "|T132330:0|t多重射击会提前设定值进行提示(单位：秒),默认0.20。",
    width = "full",
    min = 0,
    softMax = 1.5,
    step = 0.05,
...
spec:RegisterSetting( "stings_ttd", 6, {
    type = "range",
    name = "目标死亡前禁用|T132204:0|t",
    desc = "|T132204:0|t毒蛇钉刺会在目标死亡前设定时间内禁用(单位：秒),默认6,如果存在问题可以设置为0。",
    width = "full",
    min = 0,
    softMax = 10,
    step = 0.5,
...
spec:RegisterSetting( "TOC", nil, {
    type = "header",
    name = "大十字军的试炼"
} )

spec:RegisterSetting( "TOC_4_1", true, {
    type = "toggle",
    name = "4号双子战启用黑箭|T136181:0|t",
    desc = "启用后，施放黑箭|T136181:0|t,不放爆炸陷阱。",
    width = "full",
} )

spec:RegisterSetting( "ICC", nil, {
    type = "header",
    name = "冰冠堡垒"
} )
